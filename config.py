...
class Config:
    ...
    # OpenAI (для транскрибации голоса через Whisper API)
    ...
    if not DATABASE_URL:
        raise ValueError("DATABASE_URL не указан. Укажите строку подключения к PostgreSQL.")
    ...
    def validate(cls):
        ...
        required = [
            "TELEGRAM_BOT_TOKEN",
            "OPENAI_API_KEY",  # Для OpenAI Whisper API
